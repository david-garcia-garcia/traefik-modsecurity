...
- Public `failMode` string, `open` or `close`, default `open`.
...
- String `failMode` with `json:"failMode,omitempty"`. Allowed values `open` and `close` (case-insensitive; stored lowercase). Empty becomes `open` in Prepare, same pattern as `logLevel`. Invalid values fail Prepare. Reclaim hash already JSON-marshals Config, so the field participates automatically once it is on the struct.
- Store prepared `failMode` on `Plugin` in `New`. Branch in `ServeHTTP` after `recordWafFailure` and on the unhealthy early return. Do not duplicate 502 writing; one helper that writes empty 502 (reuse `http.Error(rw, "", http.StatusBadGateway)`).
...
- Tests named `NeverFailClosed` must become default-path tests plus explicit `failMode: close` cases.

## Migration Plan

- Existing deploys omit the field → fail-open, same as `main`.
- Operators who want fail-close set `failMode: close` in the middleware plugin YAML/labels.
