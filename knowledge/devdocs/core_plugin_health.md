...
- Call `RecordFailure` after `httpClient.Do` errors only when the inbound request context is still live. Skip inbound cancel and inbound deadline. Call `IsUnhealthy` before sending to the WAF.
