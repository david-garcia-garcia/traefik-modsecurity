...
- When the method is in `ignoreBodyForVerbs` and deny is false, do not send the request body to ModSecurity. Consume it, set `req.Body` to `http.NoBody`, set `ContentLength` to 0, and delete `Content-Length` before `next`, including when the WAF is already unhealthy and the request fail-opens. Do not leave the original body for the backend.
