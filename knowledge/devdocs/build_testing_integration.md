...
- `scripts/integration-tests.Tests.ps1` — main suite (`$BaseUrl` `http://localhost:8000`). Asserts no unlabeled `*-dummy-1` container, Range not 416 with labeled-app 200/206 and `Hostname`, `If-None-Match: *` / `If-Modified-Since` not 304, 16 MiB POST to `/large-body-test` not 5xx. `Allow-path throughput` runs bombardier `-c 50 -d 15s` GET and POST `/protected` when `bombardier` is on PATH (`Invoke-AllowPathBombardier`); asserts req/s > 0 and prints `BENCH stack=...`.
