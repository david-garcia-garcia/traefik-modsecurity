...
- `scripts/integration-tests.Tests.ps1` — main suite (`$BaseUrl` `http://localhost:8000`). Asserts no unlabeled `*-dummy-1` container, Range not 416 with labeled-app 200/206 and `Hostname`, `If-None-Match: *` / `If-Modified-Since` not 304, 16 MiB POST to `/large-body-test` not 5xx. Handshake inspect: forged `Upgrade: websocket` still CRS-blocks; handshake-shaped GET `/ws-echo` with a SQL-injection query is 403; live `/ws-echo` still echoes two frames (`Invoke-TcpHttpRequest` / `Invoke-WebSocketEcho`). `/threshold-test` also covers GET-with-body during fail-open and backoff resume. `/waf-5xx-test` covers sidecar HTTP 503 fail-open to whoami with body `sidecar-5xx-marker` absent. `Allow-path throughput` runs bombardier `-c 50 -d 15s` GET and POST `/protected` when `bombardier` is on PATH (`Invoke-AllowPathBombardier`); asserts req/s > 0 and prints `BENCH stack=...`.
- `scripts/integration-tests.BodySize.Tests.ps1` — large-body transport checks.
- `scripts/TestHelpers.ps1` — HTTP, WAF assertions, container names, access-log parse, WAF audit-log parse, body builder, Traefik stdout / reclaim log parse, WebSocket echo, stack compose-file map, bombardier. Helpers include `Invoke-SafeWebRequest`, `Invoke-TcpHttpRequest`, `Invoke-ChunkedHttpRequest`, `Invoke-WebSocketEcho`, `Invoke-ThresholdTestFailOpenTrip`, `Wait-ForWafAllowPath`, `Wait-ForThresholdRouteInspecting`, `Test-WafBlocking`, `Get-TraefikAccessLogEntries`, `Get-TraefikClientHost`, `Get-WafAuditLogRecords`, `Get-WafAuditClientIp`, `Get-WafAuditHost`, `Get-TraefikStdoutLines`, `Get-ReclaimLogEvents`, `Wait-ReclaimLogCount`, `Set-ReclaimDynamicTimeoutMillis`, `Get-DummyContainerName`, `Get-IntegrationStackComposeFiles`, `Invoke-AllowPathBombardier`.
- `docker-compose.test.yml` — Traefik local plugin on `:8000`, Apache CRS WAF with `httpd-vhosts.drain.conf`, labeled whoami routes, `echo-ws` (`jmalloc/echo-server`) on `/ws-echo` behind `waf-middleware`, unlabeled `waf-5xx` (`nginx:alpine` + `test-fixtures/waf-5xx.conf` always 503 `sidecar-5xx-marker`) with `/waf-5xx-test` (backoff off, failMode open), chain-retry and chain-buffer whoami routes, file-provider directory `test-dynamic/`. The `waf` service sets `REMOTEIP_HEADER=X-Real-IP`, RFC1918 `REMOTEIP_INT_PROXY`, and `MODSEC_AUDIT_LOG=/var/log/modsec_audit.log` so a deny records Traefik `ClientHost` as `REMOTE_ADDR`.
- `docker-compose.test.nginx.yml` — same Traefik/plugin/routes including `waf-5xx` / `/waf-5xx-test`; `waf` is nginx CRS with `crs-nginx/drain-origin.conf` (`unix:/tmp/modsecurity/crs-drain.sock`), `crs-nginx/proxy_backend.drain.conf.template`, `REAL_IP_HEADER=X-Real-IP` and comma-separated RFC1918 `SET_REAL_IP_FROM`.
...
- Traefik helper matches `*-traefik-1`. WAF helper prefers compose label `com.docker.compose.service=waf` (not `waf-5xx`), then name `*-waf-1$`. Threshold trip uses `docker stop -t 0` so nginx CRS cannot drain for the default 10s grace. After a prior fail-open It, call `Wait-ForThresholdRouteInspecting` before tripping again (leftover backoff still 200s). After restart, wait Docker healthy then `Wait-ForWafAllowPath` on `/protected` (backoff off) before asserting CRS inspects again.
...
- `/waf-5xx-test` fail-opens to whoami (HTTP 200, `X-Waf-Status: error`) because backoff is off and `failMode` is open. Do not treat that 200 as sidecar allow, and do not add the path to `Wait-ForAllServices` as a CRS health check. The sidecar 503 body `sidecar-5xx-marker` must not appear.
