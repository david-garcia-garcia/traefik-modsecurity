...
- When unhealthy, forward to `next` (fail-open) and optionally set the status request header to `unhealthy`. Still withhold an ignored-verb body from `next` first (`core_plugin_middleware.md`).
