...
- When unhealthy, follow prepared `failMode`: `open` forwards to `next`; `close` writes empty HTTP 502 and does not call `next`. Optionally set the status request header to `unhealthy`. Still return HTTP 400 first when `denyVerbsWithBody` lists the method and a body is present (`core_plugin_middleware.md`).
