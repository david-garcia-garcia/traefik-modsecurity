...
- Add public middleware config `failMode` (`open` or `close`). Omitted or `open` keeps today’s fail-open. `close` fail-closes. Invalid values fail Prepare.
- On WAF communication failure (sidecar transport error except inbound cancel, or sidecar 5xx), `failMode: close` returns empty HTTP 502 and does not call `next`.
...
- `core_plugin_middleware_fail-closed`: Operator `failMode` knob, default `open`, and how fail-close refuses the client when the WAF cannot inspect.

### Modified Capabilities

- `core_plugin_middleware_waf-status`: WAF communication failure SHALL fail-open unless `failMode` is `close`; then empty HTTP 502, no `next`.
- `core_plugin_middleware_health-tracker`: Already-unhealthy skip and “backoff off” WAF failure follow `failMode` instead of always calling `next`.
...
- `pkg/modsecurity/config.go` (`Config`, `CreateConfig`, `Prepare` fill and validate `failMode`).
- `pkg/modsecurity/plugin.go` (store prepared `failMode` on `Plugin`).
