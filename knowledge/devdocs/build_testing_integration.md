...
- `scripts/integration-tests.Tests.ps1` — main suite (`$BaseUrl` `http://localhost:8000`). Drain-only Its (no dummy, Range not 416) skip on whoami stacks; dummy-present skips on drain. Handshake-only skip: forged `Upgrade: websocket` still CRS-blocks; real `/ws-echo` handshake with a SQL-injection query still echoes (`Invoke-TcpHttpRequest` / `Invoke-WebSocketEcho`). `/threshold-test` also covers GET-with-body during fail-open and backoff resume. `/waf-5xx-test` covers sidecar HTTP 503 → client 502 with body `sidecar-5xx-marker` absent. `Allow-path throughput` runs bombardier `-c 50 -d 15s` GET and POST `/protected` when `bombardier` is on PATH (`Invoke-AllowPathBombardier`); asserts req/s > 0 and prints `BENCH stack=...`.
- `scripts/integration-tests.BodySize.Tests.ps1` — large-body transport checks.
- `scripts/TestHelpers.ps1` — HTTP, WAF assertions, container names, access-log parse, WAF audit-log parse, body builder, Traefik stdout / reclaim log parse, WebSocket echo, stack compose-file map, origin kind, bombardier. Helpers include `Invoke-SafeWebRequest`, `Invoke-TcpHttpRequest`, `Invoke-ChunkedHttpRequest`, `Invoke-WebSocketEcho`, `Invoke-ThresholdTestFailOpenTrip`, `Wait-ForWafAllowPath`, `Wait-ForThresholdRouteInspecting`, `Test-WafBlocking`, `Get-TraefikAccessLogEntries`, `Get-TraefikClientHost`, `Get-WafAuditLogRecords`, `Get-WafAuditClientIp`, `Get-WafAuditHost`, `Get-TraefikStdoutLines`, `Get-ReclaimLogEvents`, `Wait-ReclaimLogCount`, `Set-ReclaimDynamicTimeoutMillis`, `Get-DummyContainerName`, `Get-IntegrationStackComposeFiles`, `Get-IntegrationOriginKind`, `Invoke-AllowPathBombardier`.
- `docker-compose.test.yml` — Traefik local plugin on `:8000`, Apache CRS WAF, dummy whoami origin, labeled whoami routes, `echo-ws` (`jmalloc/echo-server`) on `/ws-echo` behind `waf-middleware`, unlabeled `waf-5xx` (`nginx:alpine` + `test-fixtures/waf-5xx.conf` always 503 `sidecar-5xx-marker`) with `/waf-5xx-test` (backoff off), file-provider directory `test-dynamic/`. The `waf` service sets `REMOTEIP_HEADER=X-Real-IP`, RFC1918 `REMOTEIP_INT_PROXY`, and `MODSEC_AUDIT_LOG=/var/log/modsec_audit.log` so a deny records Traefik `ClientHost` as `REMOTE_ADDR`.
- `docker-compose.test.nginx.yml` — same Traefik/plugin/routes including `waf-5xx` / `/waf-5xx-test`; `waf` is nginx CRS with `BACKEND=http://dummy`, `REAL_IP_HEADER=X-Real-IP` and comma-separated RFC1918 `SET_REAL_IP_FROM`.
...
- `/waf-5xx-test` is never 2xx (backoff off, sidecar 503 → plugin 502). Do not add it to `Wait-ForAllServices`.
