...
- `scripts/integration-tests.Tests.ps1` — main suite (`$BaseUrl` `http://localhost:8000`). Drain-only Its (no dummy, Range success + `Hostname`, 16 MiB POST to `/large-body-test` not 5xx) skip on whoami stacks; dummy-present and apache-whoami Range 416 skip on drain. `Allow-path throughput` runs bombardier `-c 50 -d 15s` GET and POST `/protected` when `bombardier` is on PATH (`Invoke-AllowPathBombardier`); asserts req/s > 0 and prints `BENCH stack=...`.
